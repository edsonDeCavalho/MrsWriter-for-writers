...
struct CreateNewText: View {
    ...
    @State private var selectedBook: BookClass = BookClass(title: "", descriptionBook: "", color: "#808080", type: "fanty", lastModificationDate: Date())  // State to track selected book
    ...
    var body: some View {
           NavigationStack {
               ZStack {
                   ...
                   VStack(alignment: .leading, spacing: 2) { // Adjust spacing if needed
                       TextField("chaptertitre", text: $noteTitle)
                           .background(Color.clear)
                           .padding(.horizontal)
                           .font(.system(size: 40, weight: .bold)).padding(.top, 100)
    ...
    private func saveText() {
        ...
        if(textToSave.title.isEmpty){
            textToSave.title = NSLocalizedString("emptytitle", comment: "Title is empty")
        }
